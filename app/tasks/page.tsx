...
export default function TasksPage() {
  ...
  // If no planId in URL but we have a plan, redirect to the plan page
  useEffect(() => {
    if (!planId && plan?.id && typeof window !== 'undefined') {
      router.push(`/plans/${plan.id}/tasks`);
    }
  }, [planId, plan, router]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/20 to-purple-50/10 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      {/* Header */}
      <header className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-b border-gray-200/60 dark:border-gray-800/60 px-6 py-4 shadow-sm sticky top-0 z-10">
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            <Button 
              ...
              size="sm" 
              className="p-2"
              onClick={() => router.push(planId ? `/plans/${planId}` : '/')}
            >
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        {/* Filters */}
        <div className="mb-6 p-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-xl border border-gray-200/60 dark:border-gray-700/60 shadow-sm">
          <div className="flex items-center gap-4">
